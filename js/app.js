var App = {
  ...
  canvas_w : document.getElementById('forest').width,
  canvas_h : document.getElementById('forest').height,
  grass: {
    0: [2,9,14],
    1: [3,7],
    2: [6],
  ...
  init : function () {
    ...
    // App.canvas.width = document.getElementById('game-wrap').innerWidth;
    // App.canvas.height = document.getElementById('game-wrap').innerHeight;

    App.ctx = App.canvas.getContext('2d');
    App.drawForest( App.ctx, App.canvas_w, App.canvas_h, App.step );
  }

  ,bind : function () {}

  ,drawForest : function ( ctx, width, height, step ) {
    // set forest background
    // ctx.fillStyle = '#8e8168';

    // set forest grid
    App.setGrid( ctx, width, height, step );

    // add canvas objects
    App.getObjects( ctx, width, height, step, App.grass, App.drawGrass );
    App.getObjects( ctx, width, height, step, App.cheese, App.drawCheese );
    App.getObjects( ctx, width, height, step, App.targets, App.drawTargets );
    App.getObjects( ctx, width, height, step, App.tree_green, App.drawTreesGreen );
    App.getObjects( ctx, width, height, step, App.tree_blue, App.drawTreesBlue );
    App.getObjects( ctx, width, height, step, App.tree_orange, App.drawTreesOrange );
    App.getObjects( ctx, width, height, step, App.tree_naked, App.drawTreesNaked );

    App.drawMouse( ctx, '#fffa52', '#efdf48', [440, 320] );
    App.drawMouse( ctx, '#92e8ad', '#19bc3c', [480, 320] );
    App.drawMouse( ctx, '#d6b9d2', '#ffffff', [520, 320] );
    App.drawMouse( ctx, '#c2fff9', '#8acff9', [560, 320] );
    App.drawMouse( ctx, '#fcbd4c', '#c49d30', [600, 320] );
  }


  ,setGrid : function ( ctx, width, height, step ) {
    var rows, cols;
    for (rows = 0; rows < width; rows+=step) {
      for (cols = 0; cols < height; cols+=step) {
        ctx.beginPath();
        ctx.moveTo(rows,cols);
        ctx.lineTo(rows+step,cols);
        ctx.lineTo(rows+step,cols+step);
        ctx.lineTo(rows,cols+step);
        ctx.lineWidth = 0.1;
        ctx.strokeStyle = '#c5bfb3';
        ctx.stroke();
        ctx.closePath();
        ctx.save();
      }
  ...
  ,getObjects : function ( ctx, width, height, step, obj, method_name ) {
    ...
    for (row = 0; row < len; row+=1) {
      ...
      for (col = 0; col < obj[row].length; col+=1) {
        var y = obj[row][col];
        obj_coord.push( [row, obj[row][col] ] );
      }
    }
    for (var i = 0; i < obj_coord.length; i+=1) {
      method_name( ctx, obj_coord[i][0], obj_coord[i][1], step );
    }
  }
  ...
  ,drawMouse : function ( ctx, body_color, ears_color, coords ) {
    var x = coords[0],
        y = coords[1];
  ...
  ,drawCheese : function ( ctx, x, y, size ) {
    // cheese code
  ...
  ,drawTargets : function ( ctx, x, y, size ) {
    // targets code
    ctx.save();
    ctx.translate(x*size,y*size);
  ...
  ,drawTreesGreen : function ( ctx, x, y, size ) {
    var stem = '#754e00',
        leaves = '#8cc63f';
  ...
  ,drawTreesBlue : function ( ctx, x, y, size ) {
    var stem = '#b89e6c',
        leaves = '#29abe2',
        head = '#008288';
  ...
  ,drawTreesOrange : function ( ctx, x, y, size ) {
    var stem = '#003d3d',
        leaves = '#f15a24',
        head = '#f7931e';
  ...
  ,drawTreesNaked : function ( ctx, x, y, size ) {
    var stem = '#754e00';
